...
## 11) Sẵn sàng connect GitHub (Cách A)
Theme hiện đã được chuẩn bị để merchant chỉ cần thao tác trong Shopify Admin:
1. Online Store → Themes → Add theme → Connect from GitHub
2. Chọn repository + branch chứa source theme này
3. Review theme preview và publish khi đạt yêu cầu

### Những gì đã chuẩn bị sẵn để vào preview đẹp ngay
- Bộ ảnh demo trong `assets/` (`hero-pod.svg`, `sample-shirt.svg`, `sample-mug.svg`, `sample-canvas.svg`)
- Homepage có hero visual + product sample cards có ảnh fallback
- Collection có product sample fallback khi chưa có dữ liệu catalog thực

## 12) Checklist sau khi connect từ GitHub
- Vào Theme Editor cập nhật logo/brand thật
- Gán link thật cho CTA header/home/footer
- Tạo collection và product thật để thay sample
- Kiểm tra chatbot demo if/else hoạt động
- Kiểm tra mobile CTA và vùng chatbot không chồng lấn
